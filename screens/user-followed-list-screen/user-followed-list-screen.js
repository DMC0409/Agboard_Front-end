...
import { styles } from "./user-followed-list-screen.style";
...
// 视图主组件
const userFollowedListScreen = ({ route, navigation, userToken, myUserID }) => {
    // 屏幕尺寸
    ...
    const getUserList = (pageIndex = 0) => {
        setRefreshing1(true);
        http(`following/followed/users?userID=${userID}&index=${pageIndex + 1}&pageSize=${20}`, GET, {})
            .then((res) => {
                // errorCode
                if (res.errCode) {
    ...
    return (
        <View style={styles.box}>
            <Appbar.Header mode="center-aligned" style={{ backgroundColor: '#fff' }}>
                ...
                <Appbar.Content title={userName + "的关注"} color={'#000'} titleStyle={{ fontSize: 18 }} />
            </Appbar.Header>
