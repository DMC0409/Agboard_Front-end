...
import { styles } from "./line-create-part2-for-level-screen.style";
...
import { CHILD_LABEL_ID, FRAME_DE_TIME, FRAME_LONG_MAP } from "../../config/line"

// 视图主组件
const LineCreatePart2ForLevelScreen = ({ route, navigation, userID, boardType, boardAngel }) => {
    // 屏幕尺寸
    ...
    // 每帧时长值-标签
    const [frameTimeValue, setFrameTimeValue] = useState(FRAME_DE_TIME);
    // 名称 input message
    ...
    // 默认帧长选择框
    const [showFrameTimeOptions, setShowFrameTimeOptions] = useState(false);
    ...
    useEffect(() => {
        ...
        if (route.params.draftLineInfo && !route.params.draftLineInfo.name) {
            route.params.draftLineInfo = null;
        }
        if (route.params.draftLineInfo) {
            // 如果具有这个数据，则说明说是草稿箱编辑线路
            const draftLineInfo = route.params.draftLineInfo;
            setLevelValue(draftLineInfo.level);
            setNameValue(draftLineInfo.name);
            setDetailValue(draftLineInfo.introduction);
            setFrameTimeValue(draftLineInfo.defaultTime);
        }
    }, [route.params]);
                ...
                if (res.data) {
                    ...
                    if (route.params.draftLineInfo) {
                        ...
                        setBoardAngleOptions([...res.data].map((item) => {
                            let selected = false
                            // 表单还原
                            if (item.value === route.params.draftLineInfo.angelValue) {
            ...
            .then((res) => {
                ...
                if (res.data) {
                    const list = [...res.data].map((item) => {
                        // 表单还原
                        if (route.params.draftLineInfo && route.params.draftLineInfo.labels) {
                            if (route.params.draftLineInfo.labels.indexOf(item.id) !== -1) {
                        ...
                        return {
                            id: item.id,
                            name: item.labelName,
                            selected: (route.params.draftLineInfo && route.params.draftLineInfo.labels.indexOf(item.id) !== -1) ? true : false
                        }
                    })
                    if (route.params.draftLineInfo) {
                        const routeParamsLabels = route.params.draftLineInfo.labels;
                        // “儿童”标签单独选中处理
                        if (routeParamsLabels && routeParamsLabels.length !== 0) {
                            let hasChildLabel = false;
                            routeParamsLabels.forEach((temp) => {
                        ...
                        if (routeParamsLabels) {
                            setLabelValue(routeParamsLabels);
                        }
                    }
    ...
    const saveToDraft = () => {
        ...
        http(`line/draft/update`, POST, {
            body: {
                id: route.params.draftLineInfo?.id || '',
                ...
                // 1抱石 2难度
                type: 2,
                defaultTime: Number(frameTimeValue),
            }
        })
            .then((res) => {
                // errorCode
                ...
                if (res.data) {
                    ...
                    toastMessage(route.params.draftLineInfo ? "更新成功" : `已保存至草稿箱`);
                    if (route.params.draftLineInfo) {
    ...
    const save = () => {
        ...
        // 请求
        http(route.params.draftLineInfo ? `line/draft/release` : `line/insert`, POST, {
            body: {
                id: !!(route.params.draftLineInfo && route.params.draftLineInfo.id) ? route.params.draftLineInfo.id : '',
                content: pointJsonStr || '',
                name: nameValue || '',
                video: '',
                introduction: detailValue || '',
    ...
    // 默认帧长列表视图
    const ListRenderItemForFrame = ({ item }) => {
        // 当前选择的图标
        const selectedIcon = require('../../static/img/common-icons/select-blue.png');
        // 列表项点击处理
        const handleListPress = (item) => {
            setFrameTimeValue(item);
            setShowFrameTimeOptions(false);
        }

        return <CustomOptionList
            listItem={{ id: '', name: item + ' 秒/帧', icon: frameTimeValue === item ? selectedIcon : null }}
            hideArrow={true}
            onPress={() => handleListPress(item)}
        />;
    };
    ...
    // 删除草稿线路
    const deleteLine = () => {
        http(`line/draft/delete/${route.params.draftLineInfo.id}`, GET)
            .then((res) => {
                // errorCode
                if (res.errCode) {
                    console.error('response error code: ', res.errCode);
    ...
    return (
        <View style={styles.box}>
            ...
            <ScrollView style={styles.formBox}>
                ...
                <View style={styles.formLevelBox}>
                    <Text style={styles.formName}>每帧默认时长:</Text>
                    <Text>{frameTimeValue} 秒/帧</Text>
                    <Button
                        title="更改"
                        containerStyle={styles.levelButtonBox}
                        buttonStyle={styles.levelButton}
                        titleStyle={styles.levelButtonText}
                        onPress={() => { setShowFrameTimeOptions(true) }}
                    />
                </View>
                {/* 每帧默认时长选择弹出框 */}
                <Overlay
                    isVisible={showFrameTimeOptions}
                    overlayStyle={[styles.overOverlay, { width: 0.8 * screenWidth, height: 0.7 * screenHeight }]}
                    onBackdropPress={() => setShowFrameTimeOptions(false)}
                >
                    <FlatList
                        data={Object.keys(FRAME_LONG_MAP)}
                        renderItem={ListRenderItemForFrame}
                        keyExtractor={item => item}
                        ItemSeparatorComponent={CustomOptionSeparator}
                    />
                </Overlay>
            ...
            <View style={styles.actionBox}>
                ...
                <View style={styles.actiongBoxRight}>
                    ...
                    <Button
                        title={route.params.draftLineInfo ? "更新至草稿箱" : "保存至草稿箱"}
                        containerStyle={styles.nextButtonContainer}
