...
import { styles } from "./add-course-line-part1-screen.styles";
...
// 视图主组件
const AddCourseLinePart1Screen = ({ route, navigation, boardType, BTDevice }) => {
    // 屏幕宽度
    ...
    useEffect(() => {
        if (!route.params.course.lineId) {
            ...
            // 有lineId参数更新线路
            const lineID = route.params.course.lineId || '';
                ...
                .then((res) => {
                    ...
                    if (res.data) {
                        ...
                        setLineInfo({
                            ...
                            lineId: route.params.course.lineId || ''
                        });
                        // 加载岩点布局
                        const pointsData = JSON.parse(BOARD_COORDINATE);
                        // 刷新岩点列表显示
    ...
    const handlerNextStep = () => {
        navigation.navigate('AddCourseLinePart2Screen', {
            pointJsonStr: JSON.stringify(selectedPoints),
            lineInfo: { ...lineInfo, courseID: route.params.course.id || '' }
        })
    }
            ...
            <View style={styles.titleBox}>
                <View style={styles.titleSubBox}>
                    ...
                    <View style={styles.titleTextBox}>
                        <Text style={styles.titleText}>{route.params.course.lineName || boardType.name}</Text>
                    </View>
                </View>
            </View>
            {/* 岩板布局 */}
